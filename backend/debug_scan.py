...
# Create mock OHLCV data with latest bar in market hours
def mock_load_market_data(symbol, timeframe):
    if symbol != 'NSE:NIFTY50':
        return None
    # Use a timestamp within REGULAR market hours (IST 9:15-15:30 = UTC 3:45-9:45)
    now = datetime(2026, 9, 16, 9, 0, 0, tzinfo=timezone.utc)  # 9:00 UTC = 14:30 IST
    bars = []
    base_price = 25000
    for i in range(100):
