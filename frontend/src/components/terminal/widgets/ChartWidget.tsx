...
import { sma, ema, vwap, rsi, macd, bollinger, type LinePoint } from "@/lib/indicators";
...
const toMap = (pts: LinePoint[]) => new Map(pts.map((p) => [p.time, p.value]));
...
function extractCloses(candles: Candle[]): number[] {
  return candles.map((c) => c.close);
}

function extractHLCC(candles: Candle[]): { time: number; high: number; low: number; close: number; volume: number }[] {
  return candles.map((c) => ({ time: c.time, high: c.high, low: c.low, close: c.close, volume: c.volume }));
}

export function ChartWidget({ widget }: { widget: WidgetInstance }) {
  const symbol = useWidgetSymbol(widget);
  const [range, setRange] = useState<Range>("6M");
  const [chartType, setChartType] = useState<ChartType>("candles");
  const [active, setActive] = useState<Set<Indicator>>(new Set(["SMA20", "VWAP"]));
  ...
  const { data: candles, error } = useQuery({
    ...
    queryFn: () => apiGet<Candle[]>(`/api/market/ohlcv?symbol=${encodeURIComponent(symbol)}&timeframe=${range}&bars=500`),
  ...
  const indicatorData = useMemo(() => {
    ...
    const hlcc = extractHLCC(candles);
    return {
      SMA20: active.has("SMA20") ? sma(closes, 20) : null,
      SMA50: active.has("SMA50") ? sma(closes, 50) : null,
      SMA200: active.has("SMA200") ? sma(closes, 200) : null,
      EMA20: active.has("EMA20") ? ema(closes, 20) : null,
      VWAP: active.has("VWAP") ? vwap(hlcc) : null,
      RSI: active.has("RSI") ? rsi(candles, 14) : null,
      BOLL: active.has("BOLL") ? bollinger(candles, 20, 2) : null,
  ...
  useEffect(() => {
    ...
    if (chartType === "candles") {
      ...
      mainSeries = chart.addSeries(AreaSeries, { lineColor: "#ff9900", topColor: "rgba(255,153,0,0.25)", bottomColor: "rgba(255,153,0,0)" });
    }

    mainSeries.setData(candles.map((c) => ({ time: ts(c.time), open: c.open, high: c.high, low: c.low, close: c.close })));
    ...
    const overlay = (points: LinePoint[], color: string) =>
